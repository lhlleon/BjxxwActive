...
-(void)setSlideHeadView{
    ...
    self.contentScrollView.backgroundColor = [UIColor groupTableViewBackgroundColor];
}
...
//    self.imageBackView.image = [UIImage imageNamed:@"b1"];
    ...
    for (int i = 0; i < count; i++){
        ...
        x = i * w;
        CGRect rect;
        if (self.titlesArr.count == 3) {
            if (SCREEN_5S) {
                rect = CGRectMake(([[UIScreen mainScreen]bounds].size.width / 3 ) * i + w/3 - 10, 0, w, h);
            }else{
                rect = CGRectMake(([[UIScreen mainScreen]bounds].size.width / 3 ) * i + w/3, 0, w, h);
            }
        }else{
        rect = CGRectMake(i * w, 0, w, h);
        }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.titlesArr.count == 3) {
        self.imageBackView.transform  = CGAffineTransformMakeTranslation((offsetX*(self.titleScrollView.contentSize.width / self.contentScrollView.contentSize.width)) * 2, 0);
