...
#import "wwcTableViewCell.h"


@interface WeiWanChengViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView * QBTable;
}
...
@implementation WeiWanChengViewController
...
- (void)viewDidLoad {
    ...
    //添加刷新
    [self setupRefresh];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shuaxin:) name:@"shuaxin1" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fukuan:) name:@"wwwfukuan" object:nil];//未完成界面的付款
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fukuan1:) name:@"fukuan1" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fukuan2:) name:@"fukuan2" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fukuan3:) name:@"fukuan3" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fukuan4:) name:@"fukuan4" object:nil];

    // Do any additional setup after loading the view.
}
//显示通知!
-(void)fukuan1:(NSNotification *)click
{
    self.price = click.object;
}
-(void)fukuan2:(NSNotification *)click
{
    self.titles = click.object;
}
-(void)fukuan3:(NSNotification *)click
{
    self.order = click.object;
}
-(void)fukuan4:(NSNotification *)click
{
    self.hid = click.object;
}

-(void)fukuan:(NSNotification *)click
{
    FuKuanViewController * fk = [[FuKuanViewController alloc]init];
    fk.price = self.price;
    fk.titles = self.titles;
    fk.oreder = self.order;
    fk.hid = self.hid;
    [self.navigationController pushViewController:fk animated:YES];
}

#pragma mark - 刷新的通知!(刷新界面)
...
-(void)headerRereshing
{
    ...
    NSDictionary * dic = @{@"page":page,@"uid":self.uid,@"code_id":@"2"};
    NSLog(@"%@",self.uid);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    wwcTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:iden];
    if (cell == nil) {
