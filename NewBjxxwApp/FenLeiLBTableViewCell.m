...
@interface FenLeiLBTableViewCell ()
{
    int SelfWhit;
}
...
@implementation FenLeiLBTableViewCell
...
-(void)loadViewsWithModel:(FenLeiModel *)model
{
    ...
    //活动地点
    NSString * place = [NSString string];
    
    if (model.place == NULL) {
        place = @"";
    }else{
        place = [NSString stringWithFormat:@"地点: %@",model.place];
    }
    
    //活动时间
    NSString * st = [NSString stringWithFormat:@"时间:%@~%@",model.st,model.de];
